...
from tools.utils.MathUtils import get_fifth_bit
...
# level2故障检测函数入口
# 1-passed 2-failed 3-na 4-nottested
def digital_output_error_detection(csv_file: str, result_dicts: List[Dict]):
    def error_detection(is_high: bool) -> List[int]:
        df_selected = pd.read_csv(csv_file, encoding='utf8')
        measurements_2_str = get_measurements_2(result_dicts)
        if not measurements_2_str:
            return 4, "Level2 is not tested"

        preparation_2_str = result_dicts[0].get("preparation_2")
        if not preparation_2_str:
            return 2, "The preparation for error detection has not been configured."
    ...
    logging.info("超上限")
    code, msg = error_detection(True)

    logging.info("低下限")
    code, msg = error_detection(False)
...
# ######### 数字量输出-level3,函数入口
# 1-passed 2-failed 3-na 4-nottested
def digital_output_debouncing_error_healing(csv_file: str, result_dicts: List[dict]):
    ...
    if len(preparation_3_list) == 0:
        return 4, "Level3 not tested"  # 未测试

    df_selected: DataFrame = pd.read_csv(csv_file, encoding='utf8')
    # 电压超上限
    voltage_limit(df_selected, measurements_1, uRaw_High, 'upper', measurements_3_list)
    # 电压低下限
...
def voltage_limit(df_selected: DataFrame, measurements_1: str, uRaw_Limit: str, limit_type: str,
    ...
    if len(deb_filtered_df) == 0:
        return 3, "No data exceeding the limit"
    ...
    for dfc_st_var in measurements_2_list:
        ...
        if fault_rows.empty:
            return 2, "No abnormal status was recorded"
        ...
        recovery_mask = ok_df_selected[dfc_st_var].apply(lambda x: get_fifth_bit(x) == '0')
        # bit4 = 0, 故障恢复了
